...
setup_fake_proc() {
	mkdir -p "$BOTTLEPATH/proc"
	chmod 700 "$BOTTLEPATH/proc"

	if [ ! -f "$BOTTLEPATH/proc/.loadavg" ]; then
		cat <<- EOF > "$BOTTLEPATH/proc/.loadavg"
		0.12 0.07 0.02 2/165 765
		EOF
	fi

	if [ ! -f "$BOTTLEPATH/proc/.stat" ]; then
		cat <<- EOF > "$BOTTLEPATH/proc/.stat"
	...
	if [ ! -f "$BOTTLEPATH/proc/.uptime" ]; then
		cat <<- EOF > "$BOTTLEPATH/proc/.uptime"
		124.08 932.80
		EOF
	fi

	if [ ! -f "$BOTTLEPATH/proc/.version" ]; then
		cat <<- EOF > "$BOTTLEPATH/proc/.version"
	...
	if [ ! -f "$BOTTLEPATH/proc/.vmstat" ]; then
		cat <<- EOF > "$BOTTLEPATH/proc/.vmstat"
	...
	if [ ! -f "$BOTTLEPATH/proc/.sysctl_entry_cap_last_cap" ]; then
		cat <<- EOF > "$BOTTLEPATH/proc/.sysctl_entry_cap_last_cap"
		40
		EOF
	fi
}
...
./proot \
  -L \
		...
		--bind=/sys \
		--bind="$BOTTLEPATH/proc/.loadavg:/proc/loadavg" \
		--bind="$BOTTLEPATH/proc/.stat:/proc/stat" \
		--bind="$BOTTLEPATH/proc/.uptime:/proc/uptime" \
		--bind="$BOTTLEPATH/proc/.version:/proc/version" \
		--bind="$BOTTLEPATH/proc/.vmstat:/proc/vmstat" \
		--bind="$BOTTLEPATH/proc/.sysctl_entry_cap_last_cap:/proc/sys/kernel/cap_last_cap" \
